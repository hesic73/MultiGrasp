...
class ODFieldModel():
    def __init__(self, object_model: str, batch_size: int, num_pts: int = 256, scale: float = 1.0, device='cuda', signed: bool = True):
        ...
        self.obj_names = json.load(
            open(os.path.join(basedir, "names.json"), 'r'))
        ...
        stable_rot_pts = torch.tensor(self.mesh.sample(1024)).float().to(
            self.device).unsqueeze(0).tile([self.stable_rotations.shape[0], 1, 1])
        stable_rot_pts = torch.matmul(
            self.stable_rotations, stable_rot_pts.transpose(-1, -2)).transpose(-1, -2)
        self.stable_zs = - stable_rot_pts[..., 2].min(dim=1)[0]

        self.to_local = lambda pts: undo_rotation(
            (undo_translation(pts, self.transl)), self.orient)
        self.from_local = lambda pts: do_translation(
            do_rotation(pts, self.orient), self.transl)

        with torch.no_grad():
            self.transl = torch.zeros(
                [self.batch_size, 3], device=device).float()
            self.orient = torch.eye(3, device=device).unsqueeze(
                0).tile((self.batch_size, 1, 1)).float()
            self.surface_points = self._sample_pts()

    def load_mesh(self, mesh):
        pts_surf = torch.tensor(mesh.sample(512)).float().to(
            self.device).unsqueeze(0)
        self.sampled_points = pts_surf
        self.canon_object_verts = torch.Tensor(
            mesh.vertices).to(self.device).unsqueeze(0)
    ...
    def _sample_pts(self):
        pts = torch.tensor(self.mesh.sample(self.num_pts), dtype=torch.float32,
                           device=self.device).unsqueeze(0).tile((self.batch_size, 1, 1))
    ...
    def get_obj_mesh(self, idx):
        ...
        verts = torch.tensor(mesh.vertices, device=self.device,
                             dtype=torch.float32).unsqueeze(0) * self.scale[idx]
        verts = do_translation(do_rotation(
            verts, self.orient[idx:idx+1]), self.transl[idx:idx+1])
...
def get_object_model(object_model: str, batch_size: int, scale: float = 1.0, device='cuda'):
    if not isinstance(scale, torch.Tensor) and scale == 0.0:
        scale = torch.linspace(
            0.02, 0.035, batch_size, dtype=torch.float32, device=device)
    elif not isinstance(scale, torch.Tensor):
        scale = torch.ones(
            [batch_size], dtype=torch.float32, device=device) * scale
...
class MeshModel():
    ...
    def load_mesh(self, mesh):
        ...
        self.object_face_verts = index_vertices_by_faces(
            self.canon_object_verts, self.object_faces)
    ...
    def gradient(self, points, distance):
        grad = torch.autograd.grad(
            [distance.sum()], [points], create_graph=True, allow_unused=True)[0]
        return grad / grad.norm(dim=-1, keepdim=True)

    def distance_gradient(self, points, signed=False):
        B, N, _ = points.shape
        points = points.reshape([1, -1, 3])
        dis, face_idx, _ = point_to_mesh_distance(
            points, self.object_face_verts)
        dis = torch.sqrt(dis)
        ...
        gradient = torch.autograd.grad([dis.sum()], [
                                       points], allow_unused=False, create_graph=False, retain_graph=False)[0]
        gradient = gradient / \
            (torch.norm(gradient, dim=-1, keepdim=True) + 1e-12)
