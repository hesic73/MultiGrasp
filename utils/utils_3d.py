...
def trans_rot_to_homo_matrix(translation, rotation):
    ...
    # `batch_size` x 4 x 3
    rotation = torch.concat([rotation, torch.zeros(
        [batch_size, 1, 3], device=rotation.device)], dim=-2)
    # `batch_size` x 4 x 1
    translation = torch.concat(
        [translation.unsqueeze(-1), torch.ones([batch_size, 1, 1], device=translation.device)], dim=-2)
    # `batch_size` x 4 x 4
...
def meshgrid_points(range, ticks, device='cuda', requires_grad=True):
    ...
    xx = torch.linspace(x_lo, x_hi, x_ticks, device=device,
                        requires_grad=requires_grad)
    yy = torch.linspace(y_lo, y_hi, y_ticks, device=device,
                        requires_grad=requires_grad)
    zz = torch.linspace(z_lo, z_hi, z_ticks, device=device,
                        requires_grad=requires_grad)
...
def mesh_to_pts(mesh, padding=False):
    if padding:
        ...
        pts = [
            F.pad(F.pad(torch.Tensor(m.vertices), [0, 1], mode='constant', value=1.), [
                  0, 0, 0, padding_len - m.vertices.shape[0]], mode='constant', value=0.)
            for m in mesh
        ]
